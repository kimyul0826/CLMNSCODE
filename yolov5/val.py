...
def run(
    ...
    if training:  # called by train.py
        ...
        # Directories (custom save structure)
        # Desired behavior:
        # - If --name is provided as '{experiment}_{timestamp}', save to runs/val/{experiment}/{train_experiment}/
        #   where train_experiment defaults to the full provided name (e.g., '{experiment}_{timestamp}').
        # - If --name is not provided (left as default) attempt to parse from weights path of the form
        #   '.../runs/train/{experiment}/{train_experiment}/weights/(best|last).pt'.
        # - If neither yields a result, prompt for --name.
        def _split_experiment_and_train_experiment(given_name: str):
            # If user passed a nested path-like name 'exp/train', respect it directly
            if "/" in given_name:
                exp, train_exp = given_name.split("/", 1)
                return exp, train_exp
            # If name contains an underscore, first token is treated as experiment name
            if "_" in given_name:
                experiment = given_name.split("_", 1)[0]
                train_experiment = given_name
                return experiment, train_experiment
            # Fallback: treat whole name as both layers
            return given_name, given_name

        custom_save_dir = None

        # Case 1: user provided a meaningful --name
        if name and name != "exp":
            exp_name, train_exp_name = _split_experiment_and_train_experiment(str(name))
            custom_save_dir = Path(project) / exp_name / train_exp_name

        # Case 2: derive from weights path when --name not provided
        if custom_save_dir is None:
            weights_path = weights[0] if isinstance(weights, list) else weights
            if weights_path:
                weights_str = str(Path(weights_path))
                m = re.match(r".*?/runs/train/([^/]+)/([^/]+)/weights/(best|last)\.pt$", weights_str)
                if m:
                    exp_name, train_exp_name = m.group(1), m.group(2)
                    custom_save_dir = Path(project) / exp_name / train_exp_name

        # Case 3: prompt for a name if still unresolved
        if custom_save_dir is None:
            try:
                user_input = input(
                    "--name 미지정 또는 모델 경로 인식 실패. 저장 경로를 위한 --name 을 {실험명}_{타임스탬프} 형식으로 입력하세요: "
                ).strip()
            except Exception:
                user_input = ""
            if user_input:
                exp_name, train_exp_name = _split_experiment_and_train_experiment(user_input)
                custom_save_dir = Path(project) / exp_name / train_exp_name
                name = user_input

        # Finalize save_dir (fallback to default behavior if still None)
        if custom_save_dir is None:
            save_dir = increment_path(Path(project) / name, exist_ok=exist_ok)
        else:
            save_dir = (
                increment_path(Path(custom_save_dir), exist_ok=False)
                if not exist_ok
                else Path(custom_save_dir)
            )
